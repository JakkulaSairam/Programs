...
He also defined a rule set to split the set arr[] as follows:
	...
	- split the arr[] as four subsets: sub(0, p-1), sub(p+1, q-1) sub(q+1, r-1), s 
...
for i in range(0,len(l)):
    for j in range(i+1,n):
        for k in range(j+1,n):
            if sum(l[0:i])== sum(l[i+1:j]) == sum(l[j+1:k]) ==sum(l[k+1:n]):
                #print(sum(l[0:i]),sum(l[i+1:j]),sum(l[j+1:k]),sum(l[k+1:n]))
                print("True")
                flag=1
if flag==0:
    print("False")
